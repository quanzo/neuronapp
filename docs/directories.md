...
- **`ConfigurationApp::getStartDir()`** — программный доступ к директории старта: возвращает **первую** (самую приоритетную) базовую директорию из `DirPriority` (то есть `APP_START_DIR`).
...
- определения директории старта через `getStartDir()` (используется как базовый путь по умолчанию для инструментов, если их basePath/workingDirectory не заданы явно);
- разрешения путей:
