...
- **`GrepTool`** — поиск текста внутри файлов (опционально с контекстом ±N строк через `contextLines`).
- **`ViewTool`** — чтение содержимого файлов (1-based нумерация строк).
- **`chunk_view` / `chunk_size`** — чтение файла чанками и получение размера (1-based нумерация строк).
...
### Чтение файлов: `view`, `chunk_view`, `chunk_size` (1-based)

Инструменты работы с **строками файла** используют **1-based** нумерацию (первая строка = 1), согласованную с `EditTool` и `GrepTool.lineNumber`:

- **`ViewTool`** (`view`) — чтение файла или диапазона `start_line`..`end_line` с префиксом `N|` у каждой строки.
- **`ChunckViewTool`** (`chunk_view`) — потоковое чтение чанка с лимитами `lines` и `max_chars`; в ответе `startLine` / `endLine` — 1-based.
- **`ChunckSizeTool`** (`chunk_size`) — `totalLines` / `totalLength` без построчных параметров.

Пагинация по файлу:

```json
{"tool":"chunk_view","args":{"path":"docs/tools.md","start_line":1,"max_chars":5000}}
```

Следующий чанк: `start_line = endLine + 1` из предыдущего ответа.

**Не путать** с `chat_history.*` — там индексы сообщений **0-based**.

### `GrepTool`: контекст вокруг совпадений (`contextLines`)

Свойство `contextLines` (конструктор / сеттер, не параметр LLM-вызова) — аналог `grep -C N`:

- `contextLines = 0` (по умолчанию): `lineContent` — одна строка совпадения (до 500 символов).
- `contextLines > 0`: `lineContent` — многострочный блок строк контекста до/после совпадения в формате `N|содержимое` (как в `ViewTool`), разделитель `\n`. Поле `lineNumber` — номер **строки совпадения** (1-based). Лимит блока: `min(500 * (2 * N + 1), 5000)` символов.
