...
Значения опций, оформленные как **JSON‑объект или массив** (`params`, и при необходимости другие поля), могут занимать **несколько строк** (pretty‑print): после строки `имя: {` или `имя: [` последующие строки склеиваются до получения валидного JSON или до строки следующей опции вида `идентификатор: …` (без кавычки в начале строки). Логика вынесена в `PromptOptionsParseHelper`.
