...
- **`think` / `thinking`** — переключатель режима размышлений для выполнения списка. Ключи являются синонимами, при одновременном наличии приоритет у `think`.
  - `1`/`true` включает размышления;
  - `0`/`false` отключает размышления;
  - настройка применяется ко всему запуску списка, включая агенты, переключённые внутри пунктов через `@@agent(...)`;
