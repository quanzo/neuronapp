...
JSON‑значения опций (в первую очередь `params`), начинающиеся с `{` или `[`, могут быть **многострочными**: строки блока опций склеиваются до валидного JSON или до следующей опции в формате `имя: значение` (см. `PromptOptionsParseHelper`).
...
- **`think` / `thinking`** — переключатель режима размышлений **по умолчанию** для выполнения списка. Ключи являются синонимами, при одновременном наличии приоритет у `think`.
  - `1`/`true` включает размышления;
  - `0`/`false` отключает размышления;
  - настройка применяется ко всему запуску, включая агенты, переключённые через `@@agent(...)`, если в пункте нет `@@think` / `@@thinking`;
  - для одного пункта режим можно переопределить командами `@@think` / `@@thinking` в теле задания (см. ниже);
...
- формируется общий набор параметров (`effectiveParams`) и последовательно выполняются todos:
  ...
  - в тексте todo могут встречаться команды `@@thinking` / `@@think` (синонимы):
    - переключают режим размышлений **только на время исполнения этого пункта**;
    - без скобок (`@@thinking`, `@@think`) — включить think;
    - с аргументом: `true`/`false`, `1`/`0`, `"true"`/`"false"`;
    - приоритет над `think`/`thinking` в шапке списка для данного пункта; следующий пункт снова использует режим по умолчанию (шапка или конфиг агента);
    - при нескольких командах в одном пункте учитывается **последняя** по порядку в тексте;
    - некорректный аргумент не задаёт override (наследуется think из шапки/сессии);
    - сигнатуры `@@thinking(...)` / `@@think(...)` удаляются из текста перед отправкой в LLM;
    - DTO: `ThinkingCmdDto`, `ThinkCmdDto` в `src/classes/dto/cmd/`;
