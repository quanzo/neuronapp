...
- очистка выполняется точечно по «дельте» сообщений раунда (диапазон между снимками размера истории до/после отправки);
- сообщения из дельты удаляются, если `LlmCycleHelper::isCycleRequestMsg()` распознаёт служебный user-вопрос или `LlmCycleHelper::isCycleResponseMsg()` распознаёт служебный ответ (YES/NO/WAITING или пустой текст).

Правила: явные слова **YES**, **NO**, **WAITING** (как отдельные слова латиницей, без ложных срабатываний вроде подстроки в «KNOW») и пустой текст ответа ассистента считаются служебным ответом и удаляются; неоднозначный текст ассистента (ответ «чем-то своим») не удаляется — удаляется только служебный user-вопрос; если ответа **нет вовсе** (`null/false`), из дельты будет удалён только служебный пользовательский запрос. `ToolCallMessage` (ответ инструментами) не считается служебным ответом и не удаляется.
