...
- клоны для отдельных запусков:
  - `cloneForSession(ChatHistoryCloneMode $mode)` — создаёт клон с:
    - сброшенными runtime-кешами агента и провайдера (`_agent = null`, `_provider = null`);
    - новой историей (пустой, полная копия или копия без последнего сообщения);
    - тем же `sessionKey`.
  - обычный `clone` у `ConfigurationAgent` также изолирует runtime provider state:
    если `provider` задан объектом, клон получает отдельный экземпляр provider, чтобы временные настройки
    клона не мутировали исходного агента;
...
Клоны не разделяют runtime provider state с оригиналом, поэтому временный `setThink()` или
`setThinkingBudget()` в Skill/TodoList не переписывает provider-параметры исходного агента.
